...
class PAD:
    ...
    def get_pad(self):
        return {'P': round(self.P,2)+0.1, 'A': round(self.A,2)+0.1, 'D': round(self.D,2)+0.1}
...
class EmotionToPad:
    def __init__(self):
        ...
        # self.default_value = {
        #     'happiness': {'P': 0.4, 'A': 0.3, 'D': 0.2},
        #     'sadness': {'P': -0.6, 'A': -0.5, 'D': -0.6},
        #     'anger': {'P': -0.51, 'A': 0.69, 'D': 0.35},
        #     'fear': {'P': -0.64, 'A': 0.70, 'D': -0.53},
        #     'disgust': {'P': -0.4, 'A': 0.3, 'D': 0.2},
        #     'surprise': {'P': 0.2, 'A': 0.6, 'D': 0.2}
        # }
    ...
    def calculate_virtual_emotion_center(self, emotion_intense):
        ...
        total_intensity = round(sum(active_emotions.values()),2)
        ...
        for emotion, intensity in active_emotions.items():
            ...
            if default_pad:
                P_center += round(default_pad['P'] * intensity,2)
                A_center += round(default_pad['A'] * intensity,2)
                D_center += round(default_pad['D'] * intensity,2)
    ...
    def pull_and_push(self, current_pad, virtual_center, intensity):
        ...
        new_pad = {}
        for dim in ['P', 'A', 'D']:
            # new_pad[dim] = current_pad[dim] + virtual_center[dim]* intensity
