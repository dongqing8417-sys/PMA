...
class Emotion:
    ...
    def change_emotion(self, event, bio):
    ...
    # 情绪增量
    def emotion_intense(self, new_emotion):
        intense_dict = {}
    ...
    def change_basic_need(self, bio, event):
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                # if any(value == 0 for value in basic_need_dic.values()):
                #     print(f"Attempt {attempt + 1} returned 0 value(s), retrying...")
                #     continue  # 继续进行下一个尝试
                for key, value in basic_need_dic.items():
                    if value == 0:
                        print(f"Value for '{key}' is 0, changing it to 0.5")
                        basic_need_dic[key] = 0.5
